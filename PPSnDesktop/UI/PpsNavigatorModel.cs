	...
	public class PpsNavigatorModel : LuaTable, IPpsWindowPane
	{
		...
		private sealed class SearchExpression : CurrentSearchTextExpression
		{
				...
				: base(navigator, true, searchText, searchTextOffset)
			{
				try
				{
					filterExpression = PpsDataFilterExpression.Parse(searchText, offset: searchTextOffset);
				}
				catch (Exception e)
				{
					navigator.Environment.Log.Append(PpsLogType.Debug, $"Parse of search expression failed: {e.Message}");
		...
		{
			private readonly PpsActionDefinition actionDefinition;
		...
		private sealed class LuaExpression : CurrentSearchTextExpression
		{
				...
				: base(navigator, true, searchText, searchTextOffset)
			{
				...
				else // compile to check for syntax
				{
					...
					{
						navigator.Environment.Log.Append(PpsLogType.Debug, String.Format("Syntax: {0} at {1}", e.Message, e.Index + searchTextOffset));
			...
			public override void Execute()
			{
				...
				try
				{
					...
					if (r.Count > 0)
						Navigator.Environment.Log.Append(PpsLogType.Information, String.Format("Executed: {0}", r.ToString()));
					Navigator.ClearCurrentSearchText();
				}
				catch (Exception e)
				{
		...
		public class PpsFilterView : ObservableObject, ICommand
		{
			...
			private readonly PpsViewFilter filter;

			internal PpsFilterView(PpsNavigatorModel model, PpsViewFilter filter)
			{
				this.model = model;
				this.filter = filter;
			} // ctor
		...
		public class PpsOrderView : ObservableObject, ICommand
		{
			...
			private readonly PpsNavigatorModel model;
			private readonly PpsViewOrder order;

			internal PpsOrderView(PpsNavigatorModel model, PpsViewOrder order)
			{
				this.model = model;
				this.order = order;
			} // ctor
		...
		private class RunActionCommand : ICommand
		{
			...
			private PpsActionDefinition GetActionFromParameter(object parameter)
				=> parameter as PpsActionDefinition;
		...
		private bool FilterView(object item)
			=> (item as PpsViewDefinition)?.IsVisible ?? false;
		...
		private void UpdateCurrentView(PpsViewDefinition currentView)
		{
			if (currentView == null)
			{
				currentFilter = null;
