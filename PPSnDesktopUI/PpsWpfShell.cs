...
namespace TecWare.PPSn
{
	...
	internal class PpsWpfSerivce : IPpsUIService, IPpsAsyncService
	{
		...
		private static readonly Stack<Tuple<DispatcherFrame, string>> frameStack = new Stack<Tuple<DispatcherFrame, string>>();
	...
	internal sealed class PpsWpfShellService : ObservableObject, IPpsWpfResources, IPpsShellService, IPpsShellServiceInit
	{
		...
		Task IPpsShellServiceInit.InitAsync()
		{
			...
			defaultResources.Source = shell.GetHttp().CreateFullUri("wpf/styles.xaml");
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
	...
	public sealed class PpsProxyRequest : WebRequest, IEquatable<PpsProxyRequest>
	{
		...
		{
			var request = new HttpRequestMessage(method, shell.GetHttp().CreateFullUri(originalUri.ToString()));
		...
		private HttpWebRequest CreateWebRequest()
		{
			...
			request.Credentials = shell.GetHttp().Credentials; // override the current credentials
		...
		public async override Task<WebResponse> GetResponseAsync()
		{
			...
			{
				// execute request
				response = await shell.GetHttp().SendAsync(request, HttpCompletionOption.ResponseContentRead);
